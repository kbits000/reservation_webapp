...
export async function getAllAvailableTimeSlotsForDate(dateInISO8601Format: string) {    // TODO complete this function
    try {
        ...
        // TODO import reservations settings

        const autoGeneratedReservationTimeSlots = [];
        // FROM SETTINGS --- START ---
        let start_hour = 8;
        // let end_hour =  23;
        ...
        for (let step = 0; step < 15; step++) {
            ...
            autoGeneratedReservationTimeSlots.push({
                start_time: start_time_temp.toISOString(),
                end_time: end_time_temp.toISOString()
            });
        ...
        loop1: for (const autoGeneratedReservation of autoGeneratedReservationTimeSlots) {
            ...
            const time_formatted_local_start = new Date(autoGeneratedReservation['start_time']);
            const time_formatted_local_end = new Date(autoGeneratedReservation['end_time']);
            filteredReservationsDTOList.push({
                start_time: time_formatted_local_start.toISOString(),
                end_time: time_formatted_local_end.toISOString()
            });
        }
